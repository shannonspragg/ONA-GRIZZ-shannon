...
## Extract our Posterior Estimates:
```{r extract estimates for post, echo=FALSE}
...
st_write(warp.pres.abs, "/Users/shannonspragg/ONA_GRIZZ/WARP Bears /WARP Cropped - SIP/warp_presabs_post_preds.shp")
```
...
# CCS Region ID:
ccs.postmeans.rast <- terra::rast("/Users/shannonspragg/ONA_GRIZZ/Predictor Rasters/CCS_postmeans_raster.tif" )
```

```{r make subset rasters}
...
conf.stack <- c(dist2PA.rast, dist2met.rast, tot.farms.rast, tot.farms.sq.rast, hm.dens.rast, ccs.postmeans.rast, cattle.ranching.rast, fruit.tree.nut.rast, other.animal.rast, other.crop.rast, veg.melon.rast)
...
p_conf_rast <- -3.3 + ccs.postmeans.rast + (-1.4 * dist2PA.rast) + (-1.3 * dist2met.rast) + (-0.3 * tot.farms.rast) +  (0.5 * tot.farms.sq.rast) + (0.7 * hm.dens.rast) + (-0.3 * cattle.ranching.rast) + (1.1 * fruit.tree.nut.rast) + (1.5 * other.animal.rast) + (1.6 * other.crop.rast) + (0.3 * veg.melon.rast)
