...
mini.warp.df.ps <-  data.frame(bears_presence_ps, b2pa.dist.ps, b2met.dist.ps, total.farms.ps, total.farms.ps, dom.farms.ps, CCSNAME.ps, pop.dens)
...
cv10folds <- kfold_split_stratified(K=4, x=mini.warp.df.ps$CCSNAME.ps)
...
# Now run K fold with stratefied variable:
kfold.test.ps <- rstanarm::kfold(post.pa.full, K=10, folds = cv10folds)

kfold.10.ps <- rstanarm::kfold(post.pa.full, K=10, folds = cv10folds)
