...
cv4folds <- kfold_split_stratified(K=4, x=mini.warp.df.ps$CCSNAME.ps)

cv10folds <- kfold_split_stratified(K=10, x=mini.warp.df.ps$CCSUID.ps)
...
kfold.test.ps <- rstanarm::kfold(post.pa.full, K=6, folds = cv4folds)
