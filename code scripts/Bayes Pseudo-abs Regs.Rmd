...
# Note the mean and SD's that each variable was scaled by, so we can scale the rasters later:
mean(warp.pres.abs$ds__PA_) #  3.803958
sd(warp.pres.abs$ds__PA_)  #  4.130133  ( this is just 1 sd)
mean(warp.pres.abs$Ttl_F_C)  # 175.0757
sd(warp.pres.abs$Ttl_F_C)  # 171.3236
mean(warp.pres.abs$dstn___) # 22.47916
sd(warp.pres.abs$dstn___)  #  26.46386
mean(warp.pres.abs$Hmn_Dns) # 13.50969
sd(warp.pres.abs$Hmn_Dns)  # 10.56946
mean(total.farms.sq) # 60001.49
sd(total.farms.sq) # 122225.2
...
# Here are our means and sd's that the predictors were scaled by:
mean(warp.pres.abs$ds__PA_) #  3.803958
sd(warp.pres.abs$ds__PA_)  #  4.130133  ( this is just 1 sd)
mean(warp.pres.abs$Ttl_F_C)  # 175.0757
sd(warp.pres.abs$Ttl_F_C)  # 171.3236
...
conf.rast.stack <- c(dist2pa.rast.sc, dist2metro.rast.sc, tot.farms.rast.sc, tot.farms.sq.rast.sc, hm.dens.rast.sc, ccs.varint.rast, cattle.ranching.rast, fruit.tree.nut.rast, other.animal.rast, other.crop.rast, veg.melon.rast)
...
conflict_rast <- -3.2624640 + ccs.varint.rast + (-1.3887130 * dist2pa.rast.sc) + (-1.2986664 * dist2metro.rast.sc) + (-0.2535196 * tot.farms.rast.sc) + (0.4583006 * tot.farms.sq.rast.sc) + (0.7262155 * hm.dens.rast.sc) + (-0.2987515 * cattle.ranching.rast) + (1.1384551 * fruit.tree.nut.rast) + (1.5483847 * other.animal.rast) + (1.6437851 * other.crop.rast) + (0.2613143 * veg.melon.rast)
