...
conflict_presence_ps <- warp.pres.abs$bears # Binomial bears --> for this data, 1'a are all wildlife
...
mini.warp.df.ps <-  data.frame(conflict_presence_ps, b2pa.dist.ps, b2met.dist.ps, total.farms.sq.ps, total.farms.ps, dom.farms.ps, CCSNAME.ps, pop.dens)
...
post.pa.simple <- stan_glmer(conflict_presence_ps ~ pop.dens + (1 | CCSNAME.ps), 
                  data = mini.warp.df.ps,
                  family = binomial(link = "logit"), # define our binomial glm
                  prior = t_prior, prior_intercept = int_prior, 
                  iter = 5000,
...
post.pa.compare <- stan_glmer(conflict_presence_ps ~ b2pa.dist.ps + b2met.dist.ps + dom.farms.ps + total.farms.ps + pop.dens + (1 | CCSNAME.ps), 
                            data = mini.warp.df.ps,
                            family = binomial(link = "logit"), # define our binomial glm
                            prior = t_prior, prior_intercept = int_prior,
                            iter = 5000,
...
post.pa.full <- stan_glmer(conflict_presence_ps ~ b2pa.dist.ps + b2met.dist.ps + dom.farms.ps + total.farms.ps + total.farms.sq.ps + pop.dens + (1 | CCSNAME.ps), 
                            data = mini.warp.df.ps,
                            family = binomial(link = "logit"), # define our binomial glm
...
plot.roc(conlfict_presence_ps, post.pa.simplest$fitted.values, percent=TRUE, col='red', lwd=4, print.auc=TRUE, add=TRUE, print.auc.y=40)
...
plot_model(post.pa.full, type = "pred", title="Predicted Probabilities of General Conflict Presence", terms = c("b2pa.dist.ps", "b2met.dist.ps", "pop.dens", "dom.farms.ps"))
...
post1.effects.plot <- sjPlot::plot_model(post.pa.full, 
                   axis.labels=c( "Total Farm Count ^2" ,"Total Farm Count" ,"Vegetable & Melon Farming" ,
                   ...
                   show.values=TRUE, show.p=FALSE,
                   title="Effects of Social & Environmental Variables on Wildlife Conflict")
...
names(ccs.varint.means.rast)[names(ccs.varint.means.rast) == "PostMean"] <- "CCS Varying Intercept Means"
...
names(hm.dens.rast)[names(hm.dens.rast) == "gpw_v4_population_density_adju~ountry_totals_rev11_2020_1_deg"] <- "Human Density per sq km"
...
conflict_rast <- -3.2055204 + ccs.varint.rast + (-1.3828608 * dist2pa.rast.sc) + (-1.2770144 * dist2metro.rast.sc) + (-0.2812924 * tot.farms.rast.sc) + (0.4672799 * tot.farms.sq.rast.sc) + (0.7252248 * hm.dens.rast.sc) + (-0.3491539 * cattle.ranching.rast) + (1.0034481  * fruit.tree.nut.rast) + (1.4657681 * other.animal.rast) + (1.5591872 * other.crop.rast) + (0.2028940 * veg.mel.rast)
