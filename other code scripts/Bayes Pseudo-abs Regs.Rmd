...
```
We can write a scale function to center our predictors by subtracting the means (center= TRUE) and scaling by dividing by two times their standard deviation (scale=TRUE).
...
post.pa.semi <- stan_glmer(conflict_presence_ps ~ dist.2.pa.ps + dist.2.met.ps + animal.farm.dens.ps + pop.dens + (1 | CCSNAME.ps), 
                            data = mini.warp.df.ps,
                            family = binomial(link = "logit"), # define our binomial glm
